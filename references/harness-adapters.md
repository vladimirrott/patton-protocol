...
Nested spawning differs by host configuration and session policy. A parent
worker must not assume it can create another worker, inherit a parent's
permissions, or share a mutable path safely. Prime treats nested spawning as
unavailable unless the adapter and host session explicitly prove it. A nested
mission still needs its own identity, boundary, limits, report, and independent
verification; otherwise Prime keeps the work serial.
